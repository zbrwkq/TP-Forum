<?php
require_once "../traitements/header.php";
require_once "../traitements/inscription.php";
?>

<head>
    <link rel="stylesheet" href="../pages/css/styleIC.css">
</head>

<body class="d-flex justify-content-center">
<h1 id="titre">Forum - PHP</h1>
<div id="formInscription" class="border border-white">
        <h1>Inscription</h1>
        <form method="POST">
            <div class="form-group">
                <div class="mr-2">
                    <label for="identifiant" class="form-control-lg mb-0">Identifiant</label>
                    <input type="text" class="form-control form-control-lg" id="identifiant" name="identifiant" placeholder="Entrez votre identifiant" required>    
                </div>
            </div>
            <div class="form-group d-flex">
                <div class="mr-2">
                    <label for="mdp" class="form-control-lg mb-0">Mot de passe</label>
                    <input type="password" class="form-control form-control-lg" id="mdp" name="mdp" placeholder="Mot de passe"> 
                </div>
                <div>
                    <label for="confirmationMdp" class="form-control-lg mb-0">Confirmation</label>
                    <input type="password" class="form-control form-control-lg" id="confirmationMdp" name="confirmationMdp" placeholder="Confirmez">
                </div>
            </div>
            <div class="text-center d-flex justify-content-center flex-column mt-4">
            <a href="connexion.php" class="mb-3 text-white">Tu as déjà un compte ? Se connecter</a>
                <button type="submit" class="btn btn-info btn-lg" name="envoi" value="1">S'inscrire</button>
            </div>
        </form>
    </div>
</body>